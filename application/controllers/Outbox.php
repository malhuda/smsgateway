...
class Outbox extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model(array('m_login','outbox_model'));   
    ...
    {    $data = array();
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
             ...
             $data['cek_inbox'] = $this->gammu->cek_inbox()->result_array();
              ...
              $data["count_table"]= $this->outbox_model->count_table();
              ...
              $data['biodata'] = $this->outbox_model->list_table($config['per_page'],$segment);
              $this->pagination->initialize($config); 
        $data['title'] = 'OUTBOX MESSAGE';
        $data["main"] = "outbox";
  ...
  {
    $data = $this->outbox_model->get_param($ID);
  ...
  }public function all_to_csv()
  {
    $this->load->dbutil();
     ...
     {
   $hapus = $this->db->query("delete from outbox");
   if($hapus){
    echo "<script>
        ...
        public function all_to_pdf()
  {
    ...
    $this->fpdf->Text(7,1,'Rekapitulasi Pesan Outbox Kepada ADB Se Indonesia');
    ...
    $this->fpdf->output('LAPORAN', 'I');
          ...
          if($this->input->post('action') == 'excel' && $ID != NULL){
           $ID = implode(',',$ID); 
            ...
            $query = $this->db->query("SELECT * FROM outbox WHERE ID IN ($ID)");
            $this->excel_generator->set_query($query);
            $this->excel_generator->set_header(array('SendingDateTime','DestinationNumber','TextDecoded'));
            $this->excel_generator->set_column(array('SendingDateTime','DestinationNumber','TextDecoded'));
            $this->excel_generator->set_width(array(25, 25, 100));
          ...
          $ID = implode(',',$ID); 
          $this->db->trans_start(TRUE);
          $query = $this->db->query("delete FROM outbox WHERE ID IN ($ID)");
