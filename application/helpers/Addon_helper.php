<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	{
		$CI = & get_instance();
        $data = $CI->db->get_where('SETUP_KEL',array('NO'=>$NO, 'SECT'=>$SECT))->result_array();
		return $data[0]["DESCRIP"];
	}
	
	function getWilayah($NO_PROP=0, $NO_KAB=0, $NO_KEC=0)
	{
		if($NO_KEC > 0){
			$CI = & get_instance();
			$data = $CI->db->get_where('SETUP_KEC',array('NO_PROP'=>$NO_PROP, 'NO_KAB'=>$NO_KAB, 'NO_KEC'=>$NO_KEC))->result_array();
			return $data[0]["NAMA_KEC"];
		}elseif($NO_KAB > 0){
			$CI = & get_instance();
			$data = $CI->db->get_where('SETUP_KAB',array('NO_PROP'=>$NO_PROP, 'NO_KAB'=>$NO_KAB))->result_array();
			return $data[0]["NAMA_KAB"];
		}elseif($NO_PROP > 0){
			$CI = & get_instance();
			$data = $CI->db->get_where('SETUP_PROP',array('NO_PROP'=>$NO_PROP))->result_array();
			return $data[0]["NAMA_PROP"];
		}
	}
	...
	function faces($NIK, $NO_PROP, $NO_KAB, $NO_KEC){
		$filename = "faces/".$NO_PROP."/".str_pad($NO_KAB, 2, "0", STR_PAD_LEFT)."/".str_pad($NO_KEC, 2, "0", STR_PAD_LEFT)."/".$NIK.".jpg";		
